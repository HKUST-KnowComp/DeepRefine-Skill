"""Incremental Obsidian wiki refresh for ``deeprefine apply``.

Unlike ``wiki_refresh.py`` which regenerates the entire wiki via graphify,
this module applies refinement actions as incremental edits to Obsidian-style
markdown files — preserving all human-written content and only touching
the specific wikilinks or titles that changed.

Action → edit mapping::

    insert_edge("A", "links_to", "B")  →  append ``[[B]]`` to A's .md file
    delete_edge("A", "links_to", "B")  →  remove ``[[B]]`` from A's .md file
    replace_node("A", "New Title")     →  update ``# Title`` in A's .md file
...
class ObsidianRefreshError(RuntimeError):
    """Raised when incremental Obsidian refresh cannot complete safely.
...
def apply_refinement_with_obsidian_refresh(
    graph_path: Path,
    refinement_text: str,
) -> ObsidianRefreshResult:
    """Apply refinement actions as incremental edits to Obsidian .md files.
    ...
    Returns:
        :class:`ObsidianRefreshResult` with change descriptions and the
        list of modified files.

    Raises:
        ObsidianRefreshError: If a referenced node or page file is missing,
    ...
    for action_str in actions:
        ...
        if fn == "insert_edge":
            ...
            # Use the target node's label for the wikilink text (human-readable),
    ...
    if not planned:
        return ObsidianRefreshResult(changes=[], updated_files=[])
